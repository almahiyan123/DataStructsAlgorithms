...
public class HashTableTests {
	...
	public void testContainsKey1() {
		HashTable<Thing, Thing> h = table(10, null, 1, 11, 12, 21);
		assertTrue(h.contains(thing(21)));
	}
	...
	public void testContainsKey2() {
		HashTable<Thing, Thing> h = table(10, null, 1, 11, 12, 21);
		assertTrue(h.contains(thing(1)));
		assertFalse(h.contains(thing(13)));
	}
	...
	public void testEntrySet() {
		HashTable<Thing, Thing> h = table(10, null, 1, entry(11, 11, true), 12, 21);
		Set<MapEntry<Thing, Thing>> actual = h.entrySet();

		Set<Thing> actualKeys = new HashSet<>();
		List<Thing> actualValues = new ArrayList<>();
		for (MapEntry<Thing, Thing> e : actual) {
	...
	private boolean same(HashTable<Thing, Thing> expected, HashTable<Thing, Thing> actual) {
		MapEntry<Thing, Thing>[] expectedArray = expected.getTable();
		MapEntry<Thing, Thing>[] actualArray = actual.getTable();
		...
		for (int i = 0; i < expectedArray.length; i++) {
			if (expectedArray[i] == actualArray[i]) {
				...
				return false;
					...
					expectedArray[i].isRemoved() == actualArray[i].isRemoved()) {
				continue;
			} else {
				return false;
			}
	...
	private boolean similar(HashTable<Thing, Thing> expected, HashTable<Thing, Thing> actual) {
		...
		for (MapEntry<Thing, Thing> e : expected.getTable()) {
		...
		Set<Thing> actualKeys = new HashSet<>();
		List<Thing> actualValues = new ArrayList<>();
		for (MapEntry<Thing, Thing> e : actual.getTable()) {
			if (e != null && !e.isRemoved()) {
				actualKeys.add(e.getKey());
				actualValues.add(e.getKey());
			}
	...
	private HashTable<Thing, Thing> table(int capacity, Object...objects) {
		...
		int size = 0;
		int index = 0;
		MapEntry<Thing, Thing>[] entries = new MapEntry[capacity];
		for (Object o : objects) {
			if (o instanceof MapEntry) {
				entries[index] = (MapEntry<Thing, Thing>) o;
				if (!entries[index].isRemoved()) {
					size++;
				}
	 ...
	 */
	private MapEntry<Thing, Thing> entry(int k, int v) {
	 ...
	 * Returns an entry with the given parameters
	 */
	private MapEntry<Thing, Thing> entry(int k, int v, boolean a) {
		MapEntry<Thing, Thing> e = new MapEntry<>(thing(k), thing(v));
		e.setRemoved(a);
		return e;
	}
